...
def train(epoch):
    ...
    # log
    log(epoch, losses.val, train_log_path)
...
def validate():
    ...
    for data, target in validate_loader:
        ...
        validate_n_correct += sum(list(map(lambda x: x == 0, edit_dists)))
    ...
    print("validate_n_correct: ", validate_n_correct)
    validate_edit_dist /= float(len(validate_loader.dataset)) # average edit dist
    validate_acc = validate_n_correct / float(len(validate_loader.dataset)) # accuracy
    print('\nValidation set: Average loss: {:.4f}, Average edit dist: {:.4f}, Accuracy: {:.4f}\n'.format(
        validate_loss, validate_edit_dist, validate_acc))

    print("----------------------------------------------------------------------------------------------------")
    return validate_edit_dist, validate_acc, validate_loss
...
def log(epoch, validate_loss, log_path, validate_edit_dist = None, validate_acc = None):
    results = str(epoch) + "," + str(validate_loss)
    if validate_edit_dist != None:
        results += ", " + str(validate_edit_dist)
    if validate_acc != None:
        results += ", " + str(validate_acc)
...
for epoch in range(start_epoch, args.epoch + 1):
    ...
    validate_edit_dist, validate_acc, validate_loss = validate()
    ...
    # log
    log(epoch, validate_loss, validation_log_path, validate_edit_dist, validate_acc)
#
# if not args.eval:
